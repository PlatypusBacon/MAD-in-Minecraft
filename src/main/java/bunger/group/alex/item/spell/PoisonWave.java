...
public class PoisonWave extends SpellTemplate {
    ...
    private void particle_pillar(Level level, Vec3 pos, int delay, LivingEntity user)
    {
        ...
        delay *= 3;
        // Particles
        SpellHelpers.runAfterTicks(delay, () -> spawnVolatileParticle(level, newPos, ParticleTypes.SNEEZE));
        SpellHelpers.runAfterTicks(delay+4, () -> spawnVolatileParticle(level, newPos.add(0, 0.75, 0), ParticleTypes.SNEEZE));
        SpellHelpers.runAfterTicks(delay+8, () -> spawnVolatileParticle(level, newPos.add(0, 1.5, 0), ParticleTypes.SNEEZE));
        SpellHelpers.runAfterTicks(delay+12, () -> spawnVolatileParticle(level, newPos.add(0, 2.25, 0), ParticleTypes.SNEEZE));
